...
namespace PosClient
{
    ...
    public class Client
    {
        ...
        public static void Process(int option)
        {
            switch (option)
            {
                ...
                case 4:
                    ...
                    while(true){
                        mdcd.PrintOptionMenu();
                        int opcion = mdcd.ReadOption();
                        if(opcion == 0){
                            break;
                        }
                        mdcd.Process(opcion);
        ...
        public static void EnviarClavePub()
        {
            ...
            Message msg = new Message{ From = f, To = "0", Msg = "PUBKEY " + X.RsaGetPubParsXml(rsa), Stamp = X.SignedData(X.ShaHash(f + "0" + "PUBKEY " + X.RsaGetPubParsXml(rsa)), rsa)};
            Send(socket, msg);
